...
sin = '_' + strFormat.format(Nwater) + '_' + strFormat.format(brushLen) + '_' + strFormat.format(Nbrush1) + '_' + strFormat.format(Nbrush2) + '_' + strFormat.format(oliLen) +'_' + strFormat.format(Noli) + '_' + strFormat.format(brushDist) + '_' + strFormat.format(-Amm) + '_' + strFormat.format(-All) + '_' + strFormat.format(-Aml) + '_' + strFormat.format(B) + '_' + strFormat.format(0) + '_' + strFormat.format(SimID);

snapshot = hoomd.data.gsd_snapshot("../dropOliBrushVariableSize/initialConditions/dropOliBrushFinalConf" + sin + ".gsd", frame=0)

snapshot.particles.image[:,:] = 0;
...
hoomd.dump.gsd("initialConditions/dropOliBrushSlidingFinalConf" + sout + ".gsd", group=all, period=meas_period, dynamic=["momentum"], truncate=True);
...
hoomd.dump.gsd("initialConditions/dropOliBrushSlidingFinalConf" + sout + ".gsd", group=all, overwrite=True, period=None, dynamic=["momentum"]);
